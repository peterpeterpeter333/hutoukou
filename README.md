...
- **サークル**：テーマ別コミュニティ。参加、質問、つぶやき投稿、サークル作成
- **タグ / 検索**：タグ別一覧、キーワード検索
- **匿名（ペンネーム）前提**：面倒な会員登録なしで投稿できる（cookieベースの軽量アイデンティティ）
- **立場バッジ**：当事者 / 保護者 / 支援者
...
- [ ] 本人認証（NextAuth：Google / メール）で継続利用・通知を可能に
- [ ] フォロー / タイムライン（SNS化）・回答へのコメント
- [ ] 通報・モデレーション（安心・安全の担保）
- [ ] 画像投稿・リッチテキスト
- [ ] Postgres + 全文検索（pg_trgm など）への移行
